...
def Run_NLLocSet(x_len=500, y_len=300, z_len=100):
    ...
    for mod, num, md_nm in zip(inpmod, range(len(inpmod)), mod_nm):
        ...
        # NLLOC 2 VELEST
        # RUN VELEST USING LAYER SIMPLE AND COMPLEX

    #
    #
    #             itt, vel, dep, dam = ReadVelestVel(set_outmn[i])
    #
    #             vel, dep = adjust_layer(vel, dep, step=2, dev=dev, flag_plot=False, flag_v=False)
    #
